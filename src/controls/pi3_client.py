...
head_turn, init_body_turn, travel_distance, theta_t = 0, 0, 0, 0
...
# Sample Motion Input would look like "Motion - Head_Turn: -15, Body_Turn: -12, Distance: 7, Theta: 45"

def updateMotion(motion_data):
    
    global head_turn, init_body_turn, travel_distance, theta_t
    
    motion_data = motion_data[9:]
    motion_data = motion_data.replace(" ", "")
    data_list = motion_data.split(",")
    
    head_turn = int(data_list[0].split(":")[1])
    init_body_turn = int(data_list[1].split(":")[1])
    travel_distance = int(data_list[2].split(":")[1])
    theta_t = int(data_list[3].split(":")[1])
...
def client_program():
    ...
    # Initializing servo movement stuff
    ...
    while True:
        ...
        print("Motion Data:", head_turn, init_body_turn, travel_distance, theta_t)
        print(" ")
        
        while True:
            # Set movement state base on input string values
            if current_position < travel_distance and np.abs(current_position-travel_distance) > 1e-4:
                MOVE_FORWARD = True
                MOVE_BACKWARD = False
            elif current_position > travel_distance and np.abs(current_position-travel_distance) > 1e-4:
                MOVE_FORWARD = False
                MOVE_BACKWARD = True
            else:
                MOVE_FORWARD = False
            ...
            if current_angle > init_body_turn and np.abs(current_angle-init_body_turn) > 1e-4:
                TURN_LEFT = True
                TURN_RIGHT = False
            elif current_angle < init_body_turn and np.abs(current_angle-init_body_turn) > 1e-4:
                TURN_LEFT = False
                TURN_RIGHT = True
            else:
                TURN_LEFT = False
            ...
            if TURN_RIGHT:
                for (l,i) in zip(legs,range(num_legs)):
                    l.turn(-1*(current_angle-init_body_turn))
                    current_angle = init_body_turn
            else:
                for (l,i) in zip(legs,range(num_legs)):
                    l.turn(1*(current_angle-init_body_turn)) 
                    current_angle = init_body_turn
